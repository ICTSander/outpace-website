    ...
    <section id="diensten" className="relative bg-white line-grid py-24 sm:py-32">
      <div className="mx-auto max-w-[1240px] px-6 sm:px-8">
        ...
        <AnimateIn y={20} duration={0.45}>
          <div className="flex items-start gap-6 mb-16 border-l-4 border-yellow pl-6">
            <div>
              <span className="text-[11px] font-black uppercase tracking-[0.18em] text-navy/40">
                Diensten
              </span>
        ...
        {/* Services grid — table-border trick */}
        <div className="grid gap-0 sm:grid-cols-2 lg:grid-cols-3 border-t-2 border-l-2 border-navy">
          {services.map((service, i) => (
            <AnimateIn
              ...
              duration={0.4}
            >
              <div className="group border-b-2 border-r-2 border-navy p-8 bg-white transition-colors duration-200 hover:bg-navy cursor-default h-full flex flex-col">
                {/* Number */}
                <span className="text-[11px] font-black uppercase tracking-[0.18em] text-navy/25 group-hover:text-white/25 transition-colors duration-200">
                  {service.number}
                </span>

                {/* Icon */}
                <div
                  className={`mt-4 mb-5 flex h-11 w-11 items-center justify-center ${service.iconBg} ${service.iconBg === "bg-yellow" ? "text-navy" : "text-white"}`}
                >
                  <service.icon className="h-5 w-5" strokeWidth={2} />
                </div>

                <h3 className="text-[16px] font-black tracking-tight text-navy group-hover:text-white transition-colors duration-200 font-[family-name:var(--font-heading)] uppercase">
                  {service.title}
                </h3>
                <p className="mt-3 text-[14px] leading-[1.7] text-navy/50 group-hover:text-white/55 transition-colors duration-200 flex-1">
                  {service.description}
                </p>
              </div>
            </AnimateIn>

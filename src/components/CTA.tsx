...
export default function CTA() {
  ...
  const block1Y = useTransform(scrollYProgress, [0.2, 0.7], [40, 0]);
  const block2Y = useTransform(scrollYProgress, [0.3, 0.8], [40, 0]);
  const block3Y = useTransform(scrollYProgress, [0.4, 0.9], [40, 0]);

  return (
    <section ref={ref} className="relative bg-white overflow-hidden py-16 sm:py-32 border-t-2 border-navy">
      {isMobile ? (
      ...
      <div className="relative mx-auto max-w-[1240px] px-5 sm:px-8">
        <AnimateIn y={18} duration={0.4}>
          <div className="max-w-[720px]">
            ...
            {isMobile ? (
              <h2 className="mt-3 text-[32px] font-black leading-[0.95] tracking-[-1.5px] font-[family-name:var(--font-heading)] uppercase">
            ...
            <div className="mt-6 sm:mt-8 mb-5 sm:mb-6 flex items-center gap-0">
              <div className="h-[4px] w-8 bg-yellow" />
              <div className="h-[4px] w-8 bg-accent" />
              <div className="h-[4px] w-16 bg-navy" />
            </div>

            <p className="max-w-[520px] text-[14px] sm:text-[16px] leading-[1.7] text-navy/55 font-medium">
            ...
            <div className="mt-8 sm:mt-10 flex flex-col gap-3 sm:gap-4 sm:flex-row sm:items-center">
              <Link
                href="/contact"
                className="group inline-flex items-center justify-center gap-3 border-2 border-navy bg-navy px-6 sm:px-8 py-3.5 sm:py-4 text-[12px] sm:text-[13px] font-black uppercase tracking-[0.1em] text-white transition-all duration-150 hover:bg-accent hover:border-accent active:scale-[0.97]"
              >
              ...
              <a
                ...
                className="inline-flex items-center justify-center sm:justify-start gap-2 text-[12px] sm:text-[13px] font-bold text-navy/50 transition-colors hover:text-navy"
              >
      ...
      {/* Scroll-driven geometric corner blocks — desktop only */}
      {!isMobile && (

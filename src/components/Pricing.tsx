  ...
  return (
    <section id="prijzen" className="relative bg-navy py-24 sm:py-32">
      ...
      <div className="mx-auto max-w-[1240px] px-6 sm:px-8">
        ...
        {/* Plans */}
        <div className="grid gap-0 sm:grid-cols-2 max-w-[900px] border-2 border-white/15">
          {plans.map((plan, i) => (
            <AnimateIn key={plan.name} delay={i * 0.12} y={0} x={0} duration={0.35}>
              <div
                className={`relative flex h-full flex-col p-10 border-b-2 sm:border-b-0 sm:border-r-2 border-white/15 transition-all duration-200 ${
                  plan.highlighted
                    ? "bg-accent"
                    : "bg-navy-light"
                }`}
              >
                {/* Popular badge */}
                {plan.highlighted && (
                  <div className="absolute top-0 right-0 bg-white px-4 py-1.5">
                ...
                {/* Price — BIG for high contrast */}
                <div className="border-t-2 border-white/20 pt-6 mb-8">
                  <div className="flex items-baseline gap-2">
                    <span className="text-[72px] font-black tracking-[-4px] text-white leading-none font-[family-name:var(--font-heading)]">
                      €{plan.oneTime}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 flex items-baseline gap-1">
                    <span className="text-[32px] font-black tracking-[-1.5px] text-white leading-none font-[family-name:var(--font-heading)]">
                      €{plan.monthly}
                    </span>
                  ...
                  <p className="mt-1 text-[11px] font-bold uppercase tracking-[0.1em] text-white/40">
                ...
                <Link
                  ...
                  className={`flex items-center justify-center py-4 text-[12px] font-black uppercase tracking-[0.1em] transition-all duration-150 active:scale-[0.97] border-2 ${
                    plan.highlighted
                      ...
                      : "border-white/30 text-white hover:bg-white hover:text-navy hover:border-white"
                  }`}
                >
                  Start nu
                </Link>
        ...
        <AnimateIn delay={0.3} y={16} duration={0.4}>
          <p className="mt-10 text-[13px] font-medium text-white/35">

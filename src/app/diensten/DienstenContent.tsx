...
export default function DienstenContent() {
  ...
  return (
    <div className="pt-[68px]">
      ...
      <section ref={heroRef} className="relative bg-navy overflow-hidden py-20 sm:py-28">
        ...
        <div className="relative mx-auto max-w-[1240px] px-6 sm:px-8">
          <AnimateIn y={28} duration={0.45}>
            <div className="border-l-4 border-yellow pl-6 max-w-[680px]">
              ...
              <p className="mt-5 max-w-[500px] text-[15px] leading-[1.7] text-white/55 font-medium">
        ...
        <div className="mx-auto max-w-[1240px] px-6 sm:px-8">
          <div className="space-y-0 border-t-2 border-navy">
            {services.map((service, i) => {
              const isEven = i % 2 === 0;
              return (
                <AnimateIn key={service.title} delay={0.08} y={32} rotateX={10} duration={0.48}>
                  <div className={`flex flex-col gap-0 lg:flex-row border-b-2 border-navy ${isEven ? "" : "lg:flex-row-reverse"}`}>

                    {/* Icon + title side */}
                    <div className="flex-1 p-10 lg:p-12 border-b-2 border-navy lg:border-b-0 lg:border-r-2">
                      <div className={`flex h-12 w-12 items-center justify-center mb-6 ${service.iconBg} ${iconColors[service.iconBg]}`}>
                      ...
                      <h2 className="mt-2 text-[26px] font-black tracking-tight text-navy sm:text-[30px] font-[family-name:var(--font-heading)] uppercase leading-[1.1]">
                        {service.title}
                      </h2>
                      <div className="mt-4 h-[2px] w-8 bg-navy/20" />
                      <p className="mt-4 text-[14px] leading-[1.7] text-navy/60 font-medium">
                        {service.description}
                      </p>
                      <Link
                        href="/contact"
                        className="group mt-6 inline-flex items-center gap-2 text-[12px] font-black uppercase tracking-[0.1em] text-accent transition-colors hover:text-navy"
                      >
                        Meer informatie
                    ...
                    {/* Features side */}
                    <div className="flex-1 p-10 lg:p-12 bg-surface">
                      <h3 className="mb-6 text-[11px] font-black uppercase tracking-[0.15em] text-navy/40">
                        Wat je krijgt
                      </h3>
                      <ul className="space-y-4">
                        {service.features.map((feature, fi) => (
                          <motion.li
                            ...
                            transition={{ duration: 0.35, delay: fi * 0.06, ease: [0.4, 0, 0.2, 1] }}
      ...
      {/* CTA */}
      <section className="relative bg-navy overflow-hidden py-20 sm:py-24 border-t-2 border-white/10">
        <div className="absolute top-0 left-0 right-0 h-[4px] bg-yellow" />
        <div className="relative mx-auto max-w-[1240px] px-6 sm:px-8">
          <AnimateIn y={24} duration={0.45}>
            <div className="border-l-4 border-yellow pl-6 max-w-[600px]">
              ...
              <p className="mt-4 text-[15px] text-white/50 font-medium leading-[1.7]">
                Plan een gratis kennismakingsgesprek en ontdek de mogelijkheden.
              </p>
              <Link
                href="/contact"
                className="group mt-8 inline-flex items-center gap-3 border-2 border-white bg-transparent px-8 py-4 text-[13px] font-black uppercase tracking-[0.1em] text-white transition-all duration-150 hover:bg-yellow hover:border-yellow hover:text-navy active:scale-[0.97]"
              >
                Neem contact op
